...
public class TurretAI : MonoBehaviour {
    ...
    // Implementación del patrón Singleton para el Pool de balas
    private static TurretAI instance;
    ...
    public static TurretAI Instance
    {
        get
        {
            if (instance == null)
            {
                instance = FindObjectOfType<TurretAI>();
            }
            return instance;
        }
    }
	...
	void Update () {
        ...
        // Restablece la capacidad de disparar después de un tiempo
        if (!canShoot)
        {
    ...
    public void Shoott(GameObject go)
    {
        ...
        if (bulletGO != null)
        {
            bulletGO.transform.position = muzzleMain.transform.position;
            bulletGO.transform.rotation = muzzleMain.rotation;
